...
interface BetResult {
  ...
  status: string;
  payout: string | null;
  outcome: { roll?: number; target?: number; direction?: string; multiplier?: number } | null;
  created_at: string;
}
    ...
    <div className="mx-auto max-w-lg px-4 py-8">
      ...
      <Card className="mb-6">
        <CardHeader>
          ...
          <CardDescription>
            Choose over or under a target (1-99). Roll 0.00-99.99. Win if your
        ...
        <form onSubmit={handleBet}>
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="target">Target (1-99)</Label>
              <Input
                id="target"
                type="number"
                min="1"
      ...
      {lastBet && (
        <Card>
          <CardHeader>
            <CardTitle>
              {lastBet.status === "WON" ? "You won!" : "You lost"}
            </CardTitle>
            <CardDescription>
              Roll: {lastBet.outcome?.roll ?? "—"} | Target: {lastBet.outcome?.target ?? "—"} (
              {lastBet.outcome?.direction ?? "—"})
            </CardDescription>
          </CardHeader>
          <CardContent>
            <p className="text-lg">
